 ...
 * **`keyfile-timeout=`** / **`token-timeout=`** — accept a bare integer (seconds) or any duration string accepted by Go's `time.ParseDuration` (e.g. `30s`, `2m`). Both default to 30s (`keyfile-timeout=` to `mount_timeout` when that is set), after which booster falls back to the passphrase prompt; `0` waits indefinitely.
